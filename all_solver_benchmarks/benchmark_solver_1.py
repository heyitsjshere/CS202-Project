...
def _solve_one(args_tuple):
    ...
    cmd = [
        ...
        str(script_path),
        filepath,
        "--time-limit", str(max(0.2, time_limit)),
        "--starts", str(max(1, starts)),
        "--seed", str(seed),
    ]
    ...
    try:
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=max(0.2, time_limit) + 0.25,
        )
...
def parse_args():
    ...
    p.add_argument("--csv-file", type=str, default=None,
                   help="Optional CSV path override (default: results/solver_1_results.csv)")
...
def main():
    ...
    script_path = str(root / "all_solvers" / "solver_1.py")
    ...
    tasks = [
        (str(f), script_path, args.time_limit, args.starts, args.seed, args.dataset)
        for f in files
    ]
    ...
    csv_path = Path(args.csv_file) if args.csv_file else (
        Path(args.log_file) if args.log_file else (results_dir / "solver_1_results.csv")
    )
