...
def main():
    ...
    files = sorted(
        (base_dir / "sm_j10").glob("PSP*.SCH"),
        key=lambda p: int(p.stem[3:]),
    )

    if not files:
        raise FileNotFoundError("No PSP*.SCH files found in sm_j10")

    print("Optimal benchmark mode | dataset=sm_j10 | repeat=1 | warmup=0 | optimal_time_limit=10.00s")
    counts = {
        "feasible": 0,
        "feasible_optimal": 0,
